...
(function(global){
    ...
    function create_sandbox() {
        ...
        function create_chain() {
            ...
            chainedAPI = {
                ...
                test:function (selector) {
                    ...
                    // queue $O.test() group and execute all queued test after DOMContentLoaded
                    currQueueSelector = selector;

                    if (testQueueSelectors.indexOf(currQueueSelector) === -1) testQueueSelectors[_push](currQueueSelector);
                    if (!(currQueueSelector in testQueue)) testQueue[currQueueSelector] = [];

                    // Catch cases where test() is called after the browser event has already occurred
                    // and element exists in DOM
                    if (global[_document][_readyState] === _complete &&
                        global[_document][_querySelector](currQueueSelector) !== null)
                        return chainedAPI; // no need to queue, just return chain
                    else
                        contentLoaded(global, function() {
                            var $o = instanceAPI, val;
                            for (var index= 0, length=testQueueSelectors[_length]; index < length; index++) {
                                var item = testQueueSelectors[index];
                                if (global[_document][_querySelector](item) !== null)
                                    for (var len=testQueue[item][_length], i=len;--i>=0;) {
                                        val = testQueue[item].shift();
                                        $o = $o[val[_type]].apply(null,val.args);
                                    }

                                // remove current selector from queue
                                testQueueSelectors[_splice](index, 1);
                            }

                            return nullAPI;
                        }); // run testQueue here; only run queue related to all testQueueSelectors
        ...
        nullAPI = {
            setGlobalDefaults:instanceAPI.setGlobalDefaults,
            setOptions:instanceAPI[_setOptions],
            test:function(){
                // add to testQueue - to allow multiple dependencies: eg: $O.test().test().js().wait()
                testQueue[currQueueSelector][testQueue[currQueueSelector][_length]] = {type:_test, args:[][_slice][_call](arguments)};
                return nullAPI;
            },
            js:function(){
                // add to testQueue
                testQueue[currQueueSelector][testQueue[currQueueSelector][_length]] = {type:_js, args:[][_slice][_call](arguments)};
                return nullAPI;
            },
            wait:function(){
                // add to testQueue
                testQueue[currQueueSelector][testQueue[currQueueSelector][_length]] = {type:_wait, args:[][_slice][_call](arguments)};
                return nullAPI;
            },
